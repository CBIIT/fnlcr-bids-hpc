...
def generate_mask_features(mask, **kwargs):
    ...
    Arguments:
        ...
        kwargs: dict {"bitmap", "distance_transform":cutoff, "edge", "blured_contour":sigma, "eroded"}
    ...
    if blured_ctr_str in kwargs:
        features[blured_ctr_str] = np.zeros_like(mask, dtype = 'float')
        blured = True
    ...
    for region in regions:
        ...
        if edge or eroded or blured: 
            ...
            if blured:
                blured_edge = gaussian(region_edge.astype(float), sigma = blur_sigma)
                output_loc = features[blured_ctr_str][min_row:max_row, min_col:max_col]
                np.copyto(output_loc, blured_edge , where = region.image)
