...
def testing__analysis_3d__get_new_masks():

    from .analysis_3d import get_new_masks
    from . import utils

    images_list, known_masks_list, inferred_masks_list, roi_list, models = load_data()

    new_masks_list, new_names = get_new_masks(inferred_masks_list)



    from .post_inference import calculate_metrics

    new_metrics_2d_list, new_metrics_3d_list = calculate_metrics(known_masks_list, new_masks_list, nviews=3)

    import numpy as np
    print(np.squeeze(new_metrics_2d_list[0][0,1,0,:,:]))



    from .post_inference import output_metrics

    output_metrics(new_metrics_3d_list, roi_list, models, new_names=new_names)



    from .analysis_3d import make_movies

    make_movies(roi_list, images_list, new_masks_list, models, nframes=40, known_masks_list=known_masks_list, metrics_2d_list=new_metrics_2d_list, metrics_3d_list=new_metrics_3d_list, new_names=new_names)