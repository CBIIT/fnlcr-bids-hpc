# Call this like:
#   Training:   python ~/links/models/train_unet_mask_resnet.py 0 ~/links/1-pre-processing/roi1+roi2/both_rois_images_combined_5000-ready_for_unet.npy  ~/links/1-pre-processing/roi1+roi2/both_rois_masks_combined_5000-ready_for_unet.npy
#   Predicting: python ~/links/models/train_unet_mask_resnet.py 1 ~/links/1-pre-processing/field_of_view/fov-ready_for_unet-z_first.npy ./my_prediction.npy
...
#    from segmentation_models import unet
    ...
    # Set arguments to this script
    do_prediction = bool(int(sys.argv[1]))
    inputnpyfname = sys.argv[2]
    if not do_prediction:
        targetnpyfname = sys.argv[3]
    else:
        predictnpyfname = sys.argv[3]

    # Other variables
    expandChannel = True
    reloadFlag = False
    oldmodelwtsfname = './old_model_weights.h5'
    backbone = 'resnet152'
    encoder = 'imagenet11k'
    ...
    if not do_prediction: # Train...
        ...
        # Preprocess the data
        imgs_train,imgs_mask_train = preprocess_data(do_prediction,inputnpyfname,targetnpyfname,expandChannel,backbone)
        ...
        model.compile(optimizer=Adam(lr=1e-5), loss='binary_crossentropy', metrics=['binary_crossentropy','mean_squared_error',dice_coef, dice_coef_batch, focal_loss()])
        ...
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1,patience=100, min_lr=0.001,verbose=1)
        model_es = EarlyStopping(monitor='val_loss', min_delta=0.00000001, patience=100, verbose=1, mode='auto')
        csv_logger = CSVLogger(csvfname, append=True)

        # Train the model
        model.fit(imgs_train, imgs_mask_train, batch_size=16, epochs=250, verbose=1, shuffle=True, validation_split=0.10, callbacks=[model_checkpoint, reduce_lr, model_es, csv_logger])
        #model.fit(imgs_train, imgs_mask_train, batch_size=20, epochs=250, verbose=1, shuffle=True, validation_split=0.10, callbacks=[model_checkpoint, reduce_lr, model_es, csv_logger])
        #model.fit(imgs_train, imgs_mask_train, epochs=250, verbose=1, shuffle=True, validation_split=0.10, callbacks=[model_checkpoint, reduce_lr, model_es, csv_logger])
        ...
        # Run inference
        #imgs_test_predict = model.predict(imgs_infer, batch_size=128, verbose=1)
        imgs_test_predict = model.predict(imgs_infer, batch_size=1, verbose=1)

        # Save the predicted masks
        #np.save(predictnpyfname, np.squeeze(imgs_test_predict))
        np.save(predictnpyfname, np.squeeze(np.round(imgs_test_predict).astype('uint8')))
